...
import { TableInovasiProps } from "@/app/inovasi/interfaces/table-inovasi.interface";

export default function TableInovasi({
  data,
  loading,
  openEditModal,
  openDeleteModal,
        ...
        <table className="table w-full">
          ...
          <tbody>
            {loading ? (
              ...
              paginatedData.map((inovasi, index) => (
                <tr key={inovasi.id}>
                  <td className="hidden sm:table-cell">
                    {(currentPage - 1) * itemsPerPage + index + 1}
                  </td>
                  <td className="hidden sm:table-cell">
                    {inovasi.inovasi_gambar?.length ? (
                      <div className="relative w-16 h-16">
                        <Image
                          loader={supabaseImageLoader}
                          src={
                            inovasi.inovasi_gambar?.[0]?.url_gambar ||
                            "/placeholder.png"
                          }
                          alt={
                            inovasi.inovasi_gambar?.[0]?.keterangan || "Gambar"
                          }
                          fill
                  ...
                  <td>{inovasi.judul}</td>
                  <td className="hidden md:table-cell">{inovasi.penulis}</td>
                  <td className="hidden xl:table-cell">
                    {" "}
                    {new Date(inovasi.tanggal_diterbitkan).toLocaleDateString(
                      "id-ID",
                      {
                        day: "2-digit",
                        month: "long",
                  ...
                  <td>
                    <div className="sm:hidden">
                      <button
                        ...
                        onClick={() => setMobileActionItem(inovasi.id)}
                      >
                        ⋮
                      </button>
                    </div>
                    ...
                    <div className="hidden sm:flex gap-2">
                      <button
                        ...
                        onClick={() => openEditModal(inovasi)}
                      >
                        <Edit2 size={14} /> Sunting
                      </button>
                      <button
                        className="btn btn-sm btn-outline btn-error flex items-center gap-1"
                        onClick={() => openDeleteModal(inovasi)}
                      >
                        <Trash2 size={14} /> Hapus
                      </button>
                    </div>

                    {mobileActionItem === inovasi.id && (
