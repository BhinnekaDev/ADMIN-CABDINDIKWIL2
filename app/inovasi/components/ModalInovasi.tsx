...
export default function ModalInovasi({
  onSubmit,
  modalInput,
  closeModal,
  editingItem,
  ...
  const handleRemoveImage = async () => {
    if (!modalInput.inovasi_gambar?.[0]?.url_gambar) return;

    try {
      setUploading(true);

      const url = modalInput.inovasi_gambar[0].url_gambar;
...
`}
        ...
        <div className="mb-6">
          <label className="font-medium mb-2 block">Isi Inovasi</label>
          <ReactQuill
            theme="snow"
        ...
        <div className="relative mb-6 flex flex-col items-center gap-3">
          ...
          {previewImage ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <input
                type="text"
                value={modalInput.inovasi_gambar?.[0]?.keterangan || ""}
                onChange={(e) => {
